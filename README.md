...
- 保存到本地 SQLite，不依赖云服务。
- 默认每 60 秒自动检查一次，可在“通知设置”中调整为 15～3600 秒；保存后立即应用，也可以手动立即检查或检查全部。
...
1. 选择 `微信（WxPusher）`，填写以 `SPT_` 开头的个人推送令牌。
2. 或选择 `微信（Server酱）`，填写 Server酱 SendKey。
3. 或选择 `企业微信机器人`，填写机器人 Webhook。
4. 保存后点击“发送测试通知”确认配置。
